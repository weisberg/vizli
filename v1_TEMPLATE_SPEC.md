...
**Placeholder style**: Use namespaced references:
```jinja2
{
  "width": {{ params.width }},
...
for any value injected into JSON context.
...
def build_chart(params, datasets, meta):
    ...
    chart = alt.Chart(df).mark_bar().encode(
        ...
        color=alt.Color(f'{params["color_field"]}:N',
...
```svg
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {{ params.width }} {{ params.height }}">
...
function render(container, params, datasets, meta) {
  ...
  // D3 rendering logic using datasets.table, params, etc.
...
The function receives a container DOM element, the three payload
namespaces, and renders into the container.
...
- Include text alternatives in HTML output
- Preserve readable contrast ratios
- Avoid encoding key distinctions with color alone
...
- Safe XML parsing (defusedxml or equivalent)
...
For HTML templates, choose the readiness signal mechanism.

### Step 8 — Create the sample payload

Provide one clean, representative example with 5-20 data rows.
