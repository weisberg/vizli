...
vizli normalizes all inputs into structured objects.
...
does NOT use `exec()` on arbitrary code. It imports the module, calls the
single declared function, and validates the return type. Side effects
...
6. Emit SVG string
7. For PNG: rasterize via cairosvg (default) or Playwright (fallback for
   complex SVG features like CSS animations or filters)
...
carnival — cairosvg handles most cases but some CSS features require a
real browser.
...
**Pipeline**:
1. Render Jinja2 template with payload as context
2. Bundle or resolve local assets (inline CSS/JS if configured)
3. Launch headless Chromium via Playwright
4. Disable network in browser context
5. Set viewport from canvas dimensions, scale from config
6. Set `color_scheme` for theme (controls `prefers-color-scheme`)
7. Load the HTML
8. Wait for readiness (three-tier resolution, see TEMPLATE_SPEC §8)
9. Capture console errors (surface as warnings)
10. Emit outputs:
...
- Network disabled (requests fail, not ignored)
- Console errors captured and surfaced
...
**Pipeline**:
1. Generate HTML harness page with:
   - Container `<div>` sized to canvas dimensions
   - Locally bundled D3 v7
   - Payload injected as script variables
   - User's JS module
2. Load in Playwright (same rules as html renderer)
...
author writes only the `render()` function.
...
- SVG templates: parse as XML, validate root element
...
**`light`** (default):
```python
Theme(name="light", background="#FAFAF8", foreground="#2C2C2A",
      text_primary="#2C2C2A", text_secondary="#5F5E5A",
      text_muted="#888780", border="rgba(0,0,0,0.12)",
      grid="#F1EFE8", axis="#B4B2A9",
      font_family="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif")
```

**`dark`**:
```python
Theme(name="dark", background="#1a1a18", foreground="#e8e6dc",
      text_primary="#e8e6dc", text_secondary="#c2c0b6",
      text_muted="#9c9a92", border="rgba(255,255,255,0.12)",
      grid="#2C2C2A", axis="#5F5E5A",
      font_family="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif")
```
...
| `altair` | `alt.themes.enable()` with custom theme |
| `svg` | `data-theme` attribute on root `<svg>` |
| `html` | Playwright `color_scheme` + CSS variable injection |
...
vizli uses the Anthropic Python SDK. The API key is read from
`ANTHROPIC_API_KEY` environment variable or `~/.config/vizli/config.yaml`.
...
# Full: everything
pip install "vizli[browser,altair,images,pdf,parquet,convert]"

# Development
pip install "vizli[browser,altair,images,pdf,parquet,convert,dev]"
```
